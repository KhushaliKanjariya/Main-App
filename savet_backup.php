...
foreach($sarr as $s)
{
$insq = "insert into booking (seatno,user_nm,showid) values('$s','$nm',$shid)";
...
<?php
	...
	$q = "select * from shows where show_id = $shid";
 	$r = mysqli_query($db,$q)or die(mysqli_error($db)."bb");
		...
		<?php
		$q="insert into bookingdetails(user_name,show_id,bd_seat_no,bd_amt,bd_date) values('$nm','$shid','$seat','$tot','$timestamp')";
		mysqli_query($db,$q);
